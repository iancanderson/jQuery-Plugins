/*
 * This is the forked version by iancanderson that handles precision and scale
 */
/*
 *
 ...
 * @example  $(".numeric").numeric({ precision: 12, scale: 2}); // allow 10 digits before, 2 digits after decimal point
 *
 */
...
$.fn.numeric.keypress = function(e)
{
  ...
  else
  {
    ...
    var precision = $.data(this, "numeric.precision");
    if(scale >= 0 && precision >= 0)
    {
      // If precision >= 0, make sure there's only <precision - scale> characters
      // before the decimal point.
      var digitsLeftOfDecimalAllowed = precision - scale;
      if (decimalPosition >= 0)
      {
        //If there is a decimal and the cursor is before the decimal.
        if ($.fn.getSelectionStart(this) < decimalPosition)
        {
          var lengthBeforeDecimal = decimalPosition
          allow = lengthBeforeDecimal < digitsLeftOfDecimalAllowed;
        }
      }
      else
      {
        // No decimal
        allow = this.value.length < digitsLeftOfDecimalAllowed;
      }
    }
  }
  if(!allow)
